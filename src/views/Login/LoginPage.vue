...
export default {
  ...
  setup() {
   ...
   const handleLogin = async () => {
    ...
    if (!username.value) {
      errorMessage.value = 'Username required.'; // Changed
      isLoading.value = false;
      return;
    }
    const usernamePattern = /^[a-zA-Z0-9]+$/;
    if (!usernamePattern.test(username.value)) {
      errorMessage.value = 'Incorrect username or password.'; // Changed
      isLoading.value = false;
      return;
    }

    if (!password.value) {
      errorMessage.value = 'Password required.'; // Changed
      isLoading.value = false;
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        // This part already sets the desired message for backend failures
